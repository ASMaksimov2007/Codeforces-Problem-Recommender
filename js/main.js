...
function displayProblemsInContest(contestId) {
    $.get(baseApiUrl + "contest.standings", { 'handles': handle, 'contestId': contestId, 'showUnofficial': true})
...
function getUserInfo(handle) {
    ...
    return userInformationResponse[0]; // take the only user from the response
}
...
function displayUserProfile(userInformation) {
    ...
    if (contestList.length == 0) {
        ...
        $('#current_rank_display').css('color', 'white').text("(unrated)");
    } else {
